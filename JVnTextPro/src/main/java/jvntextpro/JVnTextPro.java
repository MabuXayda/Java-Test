...
public class JVnTextPro {
	...
	/** The vn pos tagger. */
	MaxentTagger vnPosTaggerMaxent = null;
	
	/** The vn pos tagger. */
	CRFTagger vnPosTaggerCRF = null;
	...
	/**
	 * Initialize the pos tagger for Vietnamese, Maxent method.
	 *
	 * @param modelDir the model dir
	 * @return true if the initialization is successful and false otherwise
	 */
	public boolean initPosTaggerMaxent(String modelDir){
		try{
			this.vnPosTaggerMaxent = new MaxentTagger(modelDir);
		}
		catch (Exception e){
			System.out.println("Error while initializing POS TAgger");
			vnPosTaggerMaxent = null;
			return false;
		}
		return true;
	}
	...
	/**
	 * Initialize the pos tagger for Vietnamese, CRFs method.
	 *
	 * @param modelDir the model dir
	 * @return true if the initialization is successful and false otherwise
	 */
	public boolean initPosTaggerCRF(String modelDir){
		try{
			this.vnPosTaggerCRF = new CRFTagger(modelDir);
		}
		catch (Exception e){
			System.out.println("Error while initializing POS TAgger");
			vnPosTaggerCRF = null;
	...
	/**
	 * Do pos tagging, Maxent method.
	 *
	 * @param text to be tagged with POS of speech (need to have words segmented)
	 * @return the string
	 */
	public String posTaggingMaxent(String text){
		String ret = text;
		if (vnPosTaggerMaxent != null){
			ret = vnPosTaggerMaxent.tagging(text);			
		}
	...
	/**
	 * Do pos tagging, CRF method.
	 *
	 * @param text to be tagged with POS of speech (need to have words segmented)
	 * @return the string
	 */
	public String posTaggingCRF(String text){
		String ret = text;
		if (vnPosTaggerCRF != null){
			ret = vnPosTaggerCRF.tagging(text);			
		}
